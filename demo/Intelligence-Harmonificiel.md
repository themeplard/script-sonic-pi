...
### Présentation rapide de Sonic-pi et de Live coding:
### Merci d'essayer de suivre, je vous montre les éléments essenciels à la souris. (Voir les commentaires après les "###")
### Dans un premier temps, je vous montre l'arrivée des instruments gérés dans 3 boucles différents (live_loop :a,:b,:c)
...
live_loop :a do # début bloc live_loop :a
  ...
  32.times do; tick ### 32 x 1/8 Temps
    synth :piano, note: Ax.look, sustain: rrand(0.07,0.1), release: rrand(0.1,0.07), hard: rrand(0.5,0.7), pan: rrand(0,0.5),
      amp: 1.9*(knit [0,0,1,1].look,256,[0,1,1].choose,128,1,128,[0,1,1].choose,128).look ### Piano Main Droite (Notes Ax)
    sample :bd_klub, pan: rrand(0.1,-0.1), amp: 3.8 if (spread 1,16).look ^ (one_in 24) ### Kick sur les Temps 1 + Random 1/24
...
live_loop :b, sync: :a, delay: 11 do; tick # début bloc live_loop :b
  ...
  with_fx (knit :whammy,1,:none,3).look, transpose: 0, mix: 0.5, deltime: 0, max_delay_time: 0.75, pre_amp: 1.4 do
    synth :piano, note: Arb.look, attack: 0, sustain: 0.15,
      amp: 1.5,hard: rrand(0.4,0.6), pan: rrand(-0.3,-0.6) ### Piano Main Gauche (Notes Arb), FX Whammy 1/4 Notes
  end
  ...
  synth :fm, note: [Ab[0],Arb.look].look,release: rrand(0.25,0.4),
    amp: 2.9 if (spread 3,8).look ^ (one_in 12) ### Bass (Notes Ab[0], Arb)
...
live_loop :c, sync: :a, delay: 23 do; tick  # début bloc live_loop :c
  ...
  with_fx [:none,:bitcrusher,:none].choose, bits: rrand_i(4,8), mix: b, sample_rate: a do ### FX Bitcrusher
